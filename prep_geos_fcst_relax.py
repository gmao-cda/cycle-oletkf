...
def prep_geos_fcst_dir(wkdir         = None, \
    ...
    # CDA: temporary fix here
    fileRestoreSSS = "/discover/nobackup/projects/gmao/scda_iesa/cda/formal_exp/restore_files/rev_filled_remapped_cda_ESACCI_SSS_v2_20190101_20200929.nc"
    fileRestoreSST = "/discover/nobackup/projects/gmao/scda_iesa/cda/formal_exp/restore_files/rev_filled_remapped_cda_OSTIAv2_SST_v2_20190101_20200929.nc"
    for f in [fileRestoreSSS, fileRestoreSST]:
        if os.path.exists(f):
            srcPath = f
            dstPath = os.path.join(wkdir, os.path.basename(f))
            print("copy restore file: ", srcPath, "--->", dstPath) 
            shutil.copy2(srcPath, dstPath)
        else:
            raise RuntimeError("L4 restore file does not exists (",f,")")
            sys.exit(4)
