...
import os, sys, shutil, platform, glob, subprocess as sp
import datetime as dt
import argparse
...
/bin/ln -sf {self.bcsdir}/{self.bcrslv}/Gnomonic_{self.bcrslv}.dat .
 ...
 cp {self.homdir}/rev_filled_remapped_cda_ESACCI_SSS_v2_20190101_20200929.nc  INPUT/.
 cp {self.homdir}/rev_filled_remapped_cda_OSTIAv2_SST_v2_20190101_20200929.nc INPUT/.
...
def parse_cmd_line():
    ...
    parser.add_argument("expDir", help=("where to run fcst"))
    parser.add_argument("--flowDir", required=True, default=None, help=("experiment template"))
    parser.add_argument("--geosDir", required=True, default=None, help=("experiment template"))
    parser.add_argument("--site", required=True, default=None, choices=['NCCS'], help=("analDir storing analyzed files "))
    parser.add_argument("--ncpus", required=True, default=45*27, type=int, help=("total # of cpus: skylake:40, cas: 45"))
    parser.add_argument("--ncpusPerNode", required=True, default=45, type=int, help=("skylake:40, cas: 45"))
    ...
    args.flowDir       = os.path.abspath(args.flowDir)
    args.geosDir       = os.path.abspath(args.geosDir)
    args.bkgdSaveDir   = os.path.abspath(args.bkgdSaveDir)
    if args.hist2dSaveDir is not None:
        args.hist2dSaveDir = os.path.abspath(args.hist2dSaveDir)
    if args.hist3dSaveDir is not None:
        args.hist3dSaveDir = os.path.abspath(args.hist3dSaveDir)
...
def run_geos( expDir  = "/discover/nobackup/cda/projects2/tmp/20170101T00_fcst0002", \
              flowDir = "/discover/nobackup/cda/develop_space/cycle-oletkf", \
    ...
    cfg = ConfigGcmRun()
    cfg.flowdir = os.path.abspath(flowDir)

    cfg.load_g5modules(site = site )
    cfg.set_env_vars(site     = site, \
                     geosdir  = geosDir, \
                     geosbin  = os.path.join(geosDir, "bin"), \
                     geosetc  = os.path.join(geosDir, "etc"), \
                     geosutil = geosDir)
    #print(os.environ["LD_LIBRARY_PATH"])
    #print(os.environ["LD_LIBRARY64_PATH"])
    cfg.set_exp_vars(expid  = os.path.basename(expDir), \
                     expdir = expDir, \
                     homdir = expDir)
    cfg.create_exp_subdirs()
    cfg.set_exp_run_params(ncpus=ncpus, ncpus_per_node=ncpus_per_node) #skylake: 40, cascade: 45
...
if __name__ == '__main__':
    ...
    if not args.skip:
        run_geos( expDir  = args.expDir, \
              ...
              site    = args.site, \
              ncpus   = args.ncpus, \
              ncpus_per_node = args.ncpusPerNode )
    # need to move the output to differen locations
